...
Этот документ нужен, чтобы **Cursor / Claude / Antigravity / другие код‑агенты** работали одинаково и предсказуемо во **всех 5 репозиториях**:
...
- нужно, чтобы и Cursor, и Claude, и Antigravity (и другие клиенты) могли:
...
- Для стандартных задач IDE (улучшенный shell, git, навигация по коду) — достаточно **plugins** (как `superpowers` в Cursor или Antigravity).
...
Чтобы этот же гайд корректно работал для Codex:

- Используем **`AGENTS.md` в корне репозитория** как проектный onboarding для Codex.
- `CLAUDE.md` остаётся для Claude/Cursor совместимости и не должен конфликтовать с `AGENTS.md`.
- Детальные инструкции и процессы держим в `docs/agent/AGENT_GUIDE.md`, а в корневых файлах оставляем только короткие указатели.

Базовые правила для Codex в наших репозиториях:

- Перед изменениями сначала читать `CLAUDE.md` и `AGENTS.md`, затем релевантные docs.
- Делать минимальные, обратимые изменения без лишних рефакторингов.
- Перед заявлением “готово” запускать проектные проверки (test/lint/build/typecheck).
- Не выполнять продакшен-действия (deploy/ssh/restart/push) без явного запроса.

Практика синхронизации:

- Если обновили процесс для одного агента (Claude/Cursor/Antigravity/Codex), отражаем это в `AGENT_GUIDE.md`.
- Следим, чтобы `CLAUDE.md` и `AGENTS.md` ссылались на один и тот же источник истины: `docs/agent/AGENT_GUIDE.md`.
