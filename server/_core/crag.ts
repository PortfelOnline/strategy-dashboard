  ...
  try {
    const resp = await invokeLLM({
      ...
      messages: [
        ...
        { role: 'user', content: `Из статьи про "${keyword}" вытащи проверяемые утверждения: размеры госпошлин, проценты, сроки, цифры статистики, ссылки на статьи законов. Игнорируй цены 100zem.ru и общие фразы.\n\nФормат: [{"claim":"<что>","value":"<конкретное значение>","type":"fee|stat|term|law|other","snippet":"<точный фрагмент текста где это написано>"}]\n\nСТАТЬЯ (первые 8000 символов):\n${html.slice(0, 8000)}\n\nВерни ТОЛЬКО JSON-массив.` },
      ],
    });
