...
export async function generateImagePrompts(title: string, keyword?: string, h2Sections?: string[], bodyText?: string, countRequested?: number, h2Bodies?: string[]): Promise<string[]> {
  ...
  const fallback = [
    ...
    `Aerial drone view of a modern Moscow residential district — rows of colourful panel-frame apartment buildings, courtyards and playgrounds, clear sky, ${RU_SETTING}, ${QUALITY}, ${NEGATIVE}`,
    `Exterior of a typical Russian panel-frame apartment building with balconies and courtyard trees, soft daylight, ${RU_SETTING}, ${QUALITY}, ${NEGATIVE}`,
    `Top-down flat-lay of a small wooden house model and plain brass keys on a neutral desk, soft daylight, ${QUALITY}, ${NEGATIVE}`,
    `Typical Russian SNT dacha — small wooden house with porch and fenced garden plot with vegetable beds, summer light, ${RU_SETTING}, ${QUALITY}, ${NEGATIVE}`,
    `Russian private two-storey brick cottage with pitched roof and garden fence, autumn evening light, ${RU_SETTING}, ${QUALITY}, ${NEGATIVE}`,
    `Russian countryside land plot with wooden fence, tall grass and a surveyor tripod in the foreground, summer daylight, ${RU_SETTING}, ${QUALITY}, ${NEGATIVE}`,
    `Top-down flat-lay of stacked blank manila folders and a small green potted plant on a neutral desk, ${QUALITY}, ${NEGATIVE}`,
    `Bright empty modern Russian apartment interior with large balcony windows and neutral furniture, sunlit, ${RU_SETTING}, ${QUALITY}, ${NEGATIVE}`,
    `Top-down flat-lay of a glass hourglass next to a small wooden house model on a neutral desk, soft daylight, ${QUALITY}, ${NEGATIVE}`,
  ];
...
- OBJECT / ARCHITECTURE / AERIAL ONLY — NO people, NO hands, NO faces (FLUX mangles anatomy). Vary scenes, don't repeat a setting. Mix from this pool: aerial-top-down-land-plots-with-fenced-boundaries (кадастровая тема — ВЕДУЩИЙ мотив для разделов про участки/границы/карты/межевание/регистрацию), aerial-drone-Moscow-residential-district, Russian-panel-building-exterior, Russian-dacha-with-garden-plot, Russian-private-cottage-exterior, countryside-land-plot-with-surveyor-tripod (no person), bright-empty-Russian-apartment-interior, top-down-flat-lay-house-model-and-plain-keys, top-down-flat-lay-blank-manila-folders-and-plant, top-down-flat-lay-rolled-paper-with-ribbon-and-stamp, hourglass-and-house-model (для срок/время), scale-architectural-maquette-of-apartment-building, aerial-land-plots-at-golden-sunset. PREFER aerial-land/fields/flat-lay/clean-interior scenes — they are TEXT-SAFE. AVOID street-level urban scenes, canal/embankment views, courtyards, shopfronts, building facades with signage — FLUX hallucinates garbled floating text/watermarks on those. Buildings only from HIGH AERIAL (rooftops) or as a clean studio maquette. For "online/digital" sections DO NOT show any device — use a clean interior or object flat-lay. AVOID close-ups of documents/forms/plans (garbled text) and naming ANY organization (МФЦ/Росреестр/ЕГРН) on signs/walls.
...
- "Top-down flat-lay of a small wooden house model and plain brass keys on a neutral desk, soft daylight, ${QUALITY}"
  ...
  try {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const resp = await fetch(`https://api.cloudflare.com/client/v4/accounts/${acc}/ai/run/${model}`, {
        ...
        body: JSON.stringify({ image: bytes, prompt, max_tokens: 400 }),
      });
